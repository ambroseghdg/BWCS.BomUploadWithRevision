...
namespace BWCS.BomUpload.Website.Controllers
{
    ...
    public class BOMUploadControllerPREV : Controller
    {
        ...
        public JsonResult GetAllEnvironments(string fromQueryString)
        {
        ...
        [HttpPost]
        public JsonResult CheckItemExistence(string itemNumber, string environment, string site, string fromQueryString = "N")
        {
            ...
            //Session["XASlrSession"] = xaSyslk.XASession(xaUserName, xaPassword, maxIdleTime, environment, xaSystemName);

            //if (Session["XASlrSession"] != null)
            //{ 
            //    clsXA.LogoutXASyslinkSessionHandle(Session["XASlrSession"].ToString());
            //    Session["XASlrSession"] = null;
            //    Session["XASlrSession"] = clsSLR.XASession(xaUserName, xaPassword, maxIdleTime, environment, xaSystemName);
            //}
        ...
        /// <param name="rows"></param>
        /// <returns>.</returns>
        public JsonResult GetComponentsFromPDM(string itemNumber, string environment, string site, string sord, int page, int rows)
        {
            try
            {
                string loggedinUser = string.Empty;
                ...
                // Get all BOM components from SQL PDMData table for the parent item.
                PDMbomComponentList = clsXA.GetPDMComponentsFromSQL(itemNumber, strItem2DEnv, site, loggedinUser);

                // Check whether the Parent item exists in Item revision file in XA. Only valid items could be processed.
                ...
                if (PDMbomComponentList.Count > 0 && isValidXAParentItem)
                {
                    ...
                    {
                        // Check whether the item exists in Item revision file in XA. Only valid items could be processed.
                        isValidItem = ValidateItemInXA(PDMbomComponentList[i].ComponentItemNumber, environment, site, out strParentItemRevision, out strUoM, out strITTYP, out strItmDes);
                        if (isValidItem)
                        {
                    ...
                    else
                    {
                        ...
                        var BalloonChangeBOM = from xabom in XAbomComponentList
                                               ...
                                               on new { c = xabom.ComponentItemNumber.ToUpper() } equals new { c = pdmbom.ComponentItemNumber.ToUpper() }
                        ...
                        var DeleteBOM = XAbomComponentList.Except((from xabom in XAbomComponentList
                                                                   ...
                                                                   on new { a = xabom.UserSequence, c = xabom.ComponentItemNumber.ToUpper() } equals new { a = pdmbom.UserSequence, c = pdmbom.ComponentItemNumber.ToUpper() }
                                                                   select xabom).ToList());
                        ...
                        var NoChangeBOM = from xabom in XAbomComponentList
                                          join pdmbom in PDMbomComponentList
                                          on new { a = xabom.UserSequence, b = xabom.Qty, c = xabom.ComponentItemNumber.ToUpper() } equals new { a = pdmbom.UserSequence, b = pdmbom.Qty, c = pdmbom.ComponentItemNumber.ToUpper() }
                        ...
                        var ChangeBOM = from xabom in XAbomComponentList
                                        join pdmbom in PDMbomComponentList
                                        // on new { c = xabom.ComponentItemNumber } equals new { c = pdmbom.ComponentItemNumber  } // PREV
                                        on new { c = xabom.ComponentItemNumber.ToUpper(), s = xabom.UserSequence } equals new { c = pdmbom.ComponentItemNumber.ToUpper(), s = pdmbom.UserSequence }
                        ...
                        NewBOMList.Where(item => item.Flag == "1").ToList().ForEach(item => { item.Action = "Add"; item.Order = 1; item.Note = "Is not in XA BOM"; });  // Set Action to Add if new item and the item exists in item revision file.
                        ...
                        DeleteBOMList.ForEach(item => { item.Action = "Delete"; item.Order = 3; item.Note = "Item is not in SQL BOM But is in XA BOM"; });// Set Action to Delete if item exists in XA BOM but not in PDM data.
                        NoChangeBOMList.ForEach(item => { item.Action = "No Change"; item.Order = 5; item.Note = "Same in both SQL and XA"; }); // Set Action to No Change if there is no change to item usersequence or qty in PDM and XA BOM.
                ...
                Session["ParentItemRevision"] = (from item in PDMbomComponentList select item.ParentItemRevision).FirstOrDefault();
        ...
        public JsonResult CreateBOMComponent(Item componentDetail)
        {
            ...
            try
            {
                ...
                isValidItem = ValidateItemInXA(componentItem, environment, site, out strParentItemRevision, out strUoM1, out strITTYP1, out strItmDes1);
                ...
                if (isValidItem)
                {
                    ...
                    BOMUploadMasterModel clsXA = new BOMUploadMasterModel();
                    createResult = clsXA.CreateBOMComponent(site, parentItem, parentItemRevision, alternateBOMId, componentItem, componentItemRevision, usersequence, quantityPerUnit, environment, loggedinUser);
                    //HelperGeneral.SaveUserEnvInCookie(Response, "N", environment);  //BASE URL 
                }
                else
                {
        ...
        public JsonResult CreateBOMHeaderBOMComponent(Item componentDetail)
        {
            ...
            string strITTYP1 = "";
            string strItmDes1 = "";
            string strParentItemRevision = "", strItemDescription = "";
            List<Item> XAbomComponentList = new List<Item>();
            BOMUploadMasterModel clsXA = new BOMUploadMasterModel();
            ...
            try
            {
                ...
                if (isValidItem)
                {
                    ...
                    createResult = clsXA.CreateBOMComponent(site, parentItem, strParentItemRevision, alternateBOMId, componentItem, componentItemRevision, usersequence, quantityPerUnit, environment, loggedinUser);
        ...
        public ActionResult UploadBomToXA()
        {
            ...
            try
            {
                ...
                // if (Session["PDMEnvironment"] != null) { environment = Session["PDMEnvironment"].ToString(); }
                if (Session["CreateBOMHeader"] != null) { createHeader = Session["CreateBOMHeader"].ToString(); }
                if (Session["PDMParentItemRevision"] != null) { parentItemRevisionNum = Session["PDMParentItemRevision"].ToString(); } 
                ...
                logEmailBody.Append("This to notify that below issues are captured while BOM Upload to XA for Parent Item number: " + parentItemNumber + " on " + DateTime.Now + ". <br/><br/>");
                logEmailBody.Append("<table border=1>");
                logEmailBody.Append("<tr bgcolor='yellow'><th>ParentItemNo</th><th>Com.ItemNumber</th><th>Action</th><th>Env. </th><th>Site</th><th>End User</th><th>Error log</th></tr>");
                logEmailBody.Append("");
                ...
                if (createHeader == "Yes")
                {
                    int intXABOMHeaderCnt = clsXA.XAHDRCountExecuteScalar(SELECT_PSTHDR, environment, parentItemNumber, parentItemRevisionNum);
                    ...
                    {
                        headerCreationResult = clsXA.CreateBOMHeader(site, parentItemNumber, itemDescription, parentItemRevisionNum, environment, loggedinUser);
                        ...
                        {
                            logger.Info("CreateBOMHeader: " + parentItemNumber + " created in XA | Env: " + environment + " | Site: " + site);
                        }
                    }
                    else
                    {
                        logger.Info("CreateBOMHeader: " + parentItemNumber + " BOM Header already exists in XA | Env: " + environment + " | Site: " + site);
                    }
                ...
                using (var reader = new StreamReader(Request.InputStream))
                {
                    ...
                    {
                        IsDontDelete = false; 
                        int intXABOMComponentCnt = clsXA.XADTLCountExecuteScalar(SELECT_PSTDTL, environment, componentItem.ParentItemNumber, componentItem.ComponentItemNumber, componentItem.UserSequence, componentItem.Revision);
                        // // bool hasBOMCompExist = XABomComponentList.AsEnumerable().Any(row => (row.ComponentItemNumber.Trim().Equals(componentItem.ComponentItemNumber, StringComparison.OrdinalIgnoreCase))
                        // //                                                                    && (row.UserSequence.Trim().Equals(componentItem.UserSequence, StringComparison.OrdinalIgnoreCase)));
                        ...
                        if (componentItem.Action == "Add")
                        {
                            ...
                            {
                                logger.Info("Add component to XA : " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | Env: " + environment + " | Site: " + site);
                                createResult = clsXA.CreateBOMComponent(componentItem.Site, componentItem.ParentItemNumber, componentItem.ParentItemRevision, componentItem.AlternateBOMId, componentItem.ComponentItemNumber.ToUpper(), componentItem.ComponentItemRevision, componentItem.UserSequence, componentItem.Qty, componentItem.Environment, loggedinUser);
                        ...
                        else if (componentItem.Action == "Change")
                        {
                            ...
                            if (intXABOMComponentCnt > 0)
                            {
                                logger.Info("Change component to XA : " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | Env: " + environment + " | Site: " + site);
                                updateResult = clsXA.UpdateBOMComponent(componentItem.MasterCode, componentItem.Site, componentItem.ParentItemNumber, componentItem.ParentItemRevision, componentItem.AlternateBOMId, componentItem.ComponentItemNumber.ToUpper(), componentItem.ComponentItemRevision, componentItem.UserSequence, componentItem.Qty, componentItem.Environment, loggedinUser);
                        ...
                        else if (componentItem.Action == "Delete")
                        {
                            ...
                            {
                                logger.Info("Delete component from XA : " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | Env: " + environment + " | Site: " + site);
                                deleteResult = clsXA.DeleteBOMComponent(componentItem.MasterCode, componentItem.Site, componentItem.ParentItemNumber, componentItem.ParentItemRevision, componentItem.AlternateBOMId, componentItem.ComponentItemNumber.ToUpper(), componentItem.ComponentItemRevision, componentItem.UserSequence, componentItem.Qty, componentItem.Environment, loggedinUser);
                ...
                // I think it's OK to clear the SQL table after every push.    
