...
namespace BWCS.BomUpload.Website.Controllers
{
    ...
    public class BOMUploadController : Controller
    {
        ...
        public bool HasUrlParentItemRevision = false; 
        ...
        public JsonResult GetAllEnvironments(string fromQueryString, string urlParentItemRevision)
        {
            ...
            HasUrlParentItemRevision = !string.IsNullOrEmpty(urlParentItemRevision);
            ParentItemRevisionVal = string.Empty;
            if (HasUrlParentItemRevision)
        ...
        public JsonResult CheckItemExistence(string itemNumber, string environment, string site, string qsStrParentItemRevision, string fromQueryString = "N")
        {
            ...
            string strUoM; string strITTYP; string strItmDes; string strParentItemRevision = string.Empty;
            ParentItemRevisionVal = string.Empty;
            
            //HasUrlParentItemRevision = !string.IsNullOrEmpty(qsStrParentItemRevision);
            ...
            try
            {
                if ( qsStrParentItemRevision.ToString().Trim() != "GetLatestRevision" ) //  HasUrlParentItemRevision && 
                ...
                if (ItemList.Count > 0 && !string.IsNullOrEmpty(ItemList[0].ItemNumber) && !string.IsNullOrEmpty(ItemList[0].ItemDescription))
                {
                    ...
                    ParentItemRevisionVal = ItemList[0].ParentItemRevision;
                    System.Web.HttpContext.Current.Application["ItemDescription"] = ItemList[0].ItemDescription;
                    System.Web.HttpContext.Current.Application["LatestParentItemRevision"] = ItemList[0].ParentItemRevision;
                }
                else
                {
        ...
        /// <returns>Item information if item exists in the selected environment.</returns>
        [HttpPost]
        public JsonResult IsVaildHeaderParentItem(string itemNumber, string environment, string site, string qsStrParentItemRevision, string fromQueryString = "N")
        {
            string result = string.Empty;
            string loggedinUser = string.Empty;
            BOMUploadMasterModel clsXA = new BOMUploadMasterModel();
            ...
            if ( qsStrParentItemRevision.ToString().Trim() != "GetLatestRevision")
                ItemList = clsXA.GetItemDetailsByRevision(itemNumber, environment, site, loggedinUser, ParentItemRevisionVal, out strUoM, out strITTYP, out strItmDes, fromQueryString);
            else
                ItemList = clsXA.GetItemDetails(itemNumber, environment, site, loggedinUser, out strParentItemRevision, out strUoM, out strITTYP, out strItmDes, fromQueryString);
        ...
        [HttpPost]
        public JsonResult GetParentItemRevisionByParentItemSite(string itemNumber, string environment, string site, string fromQueryString = "N")
        {
            string result = string.Empty;
            string loggedinUser = string.Empty;
            BOMUploadMasterModel clsXA = new BOMUploadMasterModel();
            ...
            loggedinUser = HttpContext.User.Identity.Name.ToString().Split('\\')[1].ToUpper();
            if (loggedinUserEmail.Trim() == "") loggedinUserEmail = GetEmailForLoggedInADUser(loggedinUser);
            logger.Info("GetParentItemRevisionByParentItemSite | Logged in user : " + loggedinUser + " | Logged in user Email: " + loggedinUserEmail + Environment.NewLine);
            ...
            try
            {
                ItemList = clsXA.GetParentItemRevisionByItemSite(itemNumber, environment, site, loggedinUser, out strParentItemRevision, out strUoM, out strITTYP, out strItmDes, fromQueryString);
            ...
            catch (Exception sysex)
            {
                Item objItem = new Item();
                objItem.ItemNumber = itemNumber;
                objItem.ItemDescription = "GetParentItemRevisionByParentItemSite - Systemlink service call error at " + DateTime.Now.ToString() + " | " + sysex.Message;
                objItem.CompleteDescription = "GetParentItemRevisionByParentItemSite - Systemlink service call error at " + DateTime.Now.ToString() + " | " + sysex.Message;
                objItem.DrawingNumber = string.Empty;
                ItemList.Add(objItem);

                logger.Error("GetParentItemRevisionByParentItemSite - Systemlink service call error at " + DateTime.Now.ToString() + sysex.Message + System.Environment.NewLine);
                logger.Info("GetParentItemRevisionByParentItemSite| loggedinUser: " + loggedinUser + " | LoggedinUsereMail: " + loggedinUserEmail);
                SendEmailAlertNotificationBOMUploadtoXASubject("Upload Bom To XA | GetParentItemRevisionByParentItemSite - Systemlink service call Error: " + sysex.Message, loggedinUserEmail, itemNumber, environment);
            }
            finally
            { }
            var Results = ItemList.Select(
        ...
        public JsonResult GetSingleLevelBOMComponentsByParentItemRevision(string itemNumber, string environment, string site, string parentitemrevision, string sord, int page, int rows)
        {
            try
            {
                ...
                loggedinUser = HttpContext.User.Identity.Name.ToString().Split('\\')[1].ToUpper();
                if (!string.IsNullOrEmpty(loggedinUser) && string.IsNullOrEmpty(loggedinUserEmail.Trim())) { loggedinUserEmail = GetEmailForLoggedInADUser(loggedinUser); }

                logger.Info("GetSingleLevelBOMComponentsByParentItemRevision | Logged in user : " + loggedinUser + Environment.NewLine);
                logger.Info("GetSingleLevelBOMComponentsByParentItemRevision begins... for itemnumber  " + itemNumber + " in " + environment + " environment, Site: " + site);

                if ((string.IsNullOrEmpty(site) || site == "null") && (!string.IsNullOrEmpty(environment)))
                {
                    BOMUploadMasterModel objSite = new BOMUploadMasterModel();
                    site = objSite.GetSiteByEnvironment(environment);
                    logger.Info("GetSingleLevelBOMComponentsByParentItemRevision|Site pulled from SQL table : " + site + Environment.NewLine);
                }

                ////if (parentitemrevision == "undefined")
                ////{
                ////    object objlatRev = System.Web.HttpContext.Current.Application["LatestParentItemRevision"];
                ////    if (objlatRev != null)
                ////        parentitemrevision = (string)objlatRev;
                ////}

                ItemList = clsXA.GetParentItemRevisionByItemSite(itemNumber, environment, site, loggedinUser, out strParentItemRevision, out strUoM, out strITTYP, out strItmDes, "");

                if (ItemList.Count > 0 && !string.IsNullOrEmpty(ItemList[0].ItemNumber) && !string.IsNullOrEmpty(ItemList[0].ItemDescription))
                {
                    logger.Info("ParentItemRevision for Item " + itemNumber + " found in XA!");

                    ParentItemRevisionVal = ItemList[0].ParentItemRevision;
                    parentitemrevision = ItemList[0].ParentItemRevision;
                    System.Web.HttpContext.Current.Application["LatestParentItemRevision"] = ItemList[0].ParentItemRevision;
                }
                else
                { 
                    System.Web.HttpContext.Current.Application["LatestParentItemRevision"] = "";
                    parentitemrevision = "";
                }

                bomComponentList = clsXA.GetAllSingleLevelBOMRevision(itemNumber, environment, site, parentitemrevision, loggedinUser);
                ...
                if (bomComponentList.Count > 0)
                {
                    ...
                    strUserActivity = "Searched BOM components are loaded from XA for Parent Item: " + itemNumber;
                }
                else
                {
                    Session["Site"] = site;
                    ...
                    strUserActivity = "Searched BOM components are not available in XA for Parent Item: " + itemNumber;
                    // TO DO implement Create Header process and Email notification in case any error 
                }
                Session["Environment"] = environment;

                HelperGeneral.RecordBOMUploadToXAUserAction(loggedinUserEmail, strUserActivity, itemNumber, "Site: " + site, environment);

                logger.Info("GetSingleLevelBOMComponentsByParentItemRevision|Retrieved single level BOM from XA");
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { success = false, message = "GetSingleLevelBOMComponentsByParentItemRevision|Failed to retrieve single level BOM components from XA !" }, JsonRequestBehavior.AllowGet);
            }
        ...
        public JsonResult ListComparedComponentsResultByPDMandXAData(string itemNumber, string environment, string site, string urlSQLParentItemRevision, string sord, int page, int rows)
        {
            try
            {
                ...
                string ApplicationLatestParentItemRevision = string.Empty; 
                ...
                ParentItemRevisionVal = urlSQLParentItemRevision.ToString().Trim(); 

                object objlatRev = System.Web.HttpContext.Current.Application["LatestParentItemRevision"];
                if (objlatRev != null)
                    ApplicationLatestParentItemRevision = (string)objlatRev;

                ApplicationLatestParentItemRevision = ParentItemRevisionVal.Length > 0 ? ParentItemRevisionVal : ApplicationLatestParentItemRevision;
                ...
                if (!HasUrlParentItemRevision && urlSQLParentItemRevision == "GetLatestRevision") // HasUrlParentItemRevision &&  
                { 
                    isValidXAParentItem = ValidateItemInXA(itemNumber, environment, site, out strParentItemRevision, out strUoM, out strITTYP, out strItmDes);
                    // If there is no ParentItemRevision passed in from URL, then get the latest revision from XA and use it for SQL data retrieval and comparision.  
                    ApplicationLatestParentItemRevision = strParentItemRevision;   
                }
                else
                ...
                if (PDMbomComponentList.Count > 0 && isValidXAParentItem)
                {
                    ...
                    {
                        // Validation in ITMRVA only  
                        isValidItem = ValidateItemInXARevision(PDMbomComponentList[i].ComponentItemNumber, environment, site, PDMbomComponentList[i].ComponentItemRevision, out strUoM, out strITTYP, out strItmDes);
                        ...
                        if (isValidItem)
                        {
                            ...
                            // it is not required, NOW Revision avalalble in both SQL and XA 
                            ////PDMbomComponentList[i].ParentItemRevision = strParentItemRevision;
                            ////PDMbomComponentList[i].ComponentItemRevision = strCompParentItemRevision;
                    ...
                    // Get all BOM components from XA for the parent item and confirmed ParentItemRevisionVal 

                    ////if (urlSQLParentItemRevision == "GetLatestRevision")
                    ////    XAbomComponentList = clsXA.GetAllSingleLevelBOM(itemNumber, environment, site, loggedinUser, "Y");
                    ////else

                    XAbomComponentList = clsXA.GetAllSingleLevelBOMRevision(itemNumber, environment, site, ApplicationLatestParentItemRevision, loggedinUser, "Y");
                ...
                if (PDMbomComponentList.Count > 0)
                {
                    strUserActivity = "PDM data retrieved from SQL and Listed Compared Components Result By SQL PDM and XA Data for Parent Item: " + itemNumber;
                }
                else
                {   strUserActivity = "No PDM data found in SQL for Parent Item: " + itemNumber + " | Env.: " + environment + " | Site: " + site;
                } 
                HelperGeneral.RecordBOMUploadToXAUserAction(loggedinUserEmail, strUserActivity, itemNumber, "Site: " + site, environment); 
        ...
        public JsonResult UpdateBOMComponent(Item componentDetail)
        {
            ...
            try
            {
                ...
                if (updateResult.ToUpper().Contains("SUCCESS"))
                    strUserActivity = "BOM Component updated: " + componentItem + " in XA | Comp. Item Revision " + componentItemRevision;
                else
                    strUserActivity = "BOM Component update failed: " + componentItem + " in XA | Comp. Item Revision " + componentItemRevision;
            }
            catch (Exception ex)
            {
                strUserActivity = "BOM Component " + componentItem + " update failed in XA for Parent Item: " + parentItem + " in XA | Env: " + environment + " | Site: " + site;
                logger.Error(DateTime.Now.ToString() + ex.Message + System.Environment.NewLine);
                errorMessage = "Error :" + ex.Message;
                return Json(errorMessage, JsonRequestBehavior.AllowGet);
            }

            HelperGeneral.RecordBOMUploadToXAUserAction(loggedinUserEmail, strUserActivity, parentItem, "Site: " + site, environment);   
        ...
        public JsonResult DeleteBOMComponent(string id)
        {
            ...
            try
            {
                ...
                if (deleteResult.ToUpper().Contains("SUCCESS"))  
                    strUserActivity = "BOM Component deleted: " + componentItem + " in XA | Comp. Item Revision " + componentItemRevision;
                else
                    strUserActivity = "BOM Component delete failed: " + componentItem + " in XA | Comp. Item Revision " + componentItemRevision; 
            }
            catch (Exception ex)
            {
                strUserActivity = "BOM Component " + componentItem + " delete failed in XA for Parent Item: " + parentItem + " in XA | Env: " + environment + " | Site: " + site;
                logger.Error(DateTime.Now.ToString() + ex.Message + System.Environment.NewLine);
        ...
        public JsonResult CreateBOMComponent(Item componentDetail)
        {
            ...
            try
            {
                ...
                if (isValidItem)
                {
                    ...
                    if (createResult.ToUpper().Contains("SUCCESS"))   
                        strUserActivity = "BOM Component created: " + componentItem + " in XA | Comp. Item Revision " + strCompParentItemRevision;
                    else
                        strUserActivity = "BOM Component creation failed: " + componentItem + " in XA | Comp. Item Revision " + strCompParentItemRevision; 

                    logger.Info("CreateBOMComponent result: " + createResult + Environment.NewLine);
                }
                else
                {
                    strUserActivity = "BOM Component " + componentItem + " does not exist in XA for Parent Item: " + parentItem + " in XA | Env: " + environment + " | Site: " + site;
                    logger.Info("Component " + componentItem + " does not exist in XA " + Environment.NewLine);
                    errorMessage = "Component Item " + componentItem + " does not exist in XA. Create the item using New Item Creation (NIC) tool and upload the BOM!";
                    
                    // Suppressed error to User, since email notification sent to user and admin.                
        ...
        public JsonResult CreateBOMHeaderBOMComponent(Item componentDetail)
        {
            ...
            try
            {
                ...
                if (XAbomComponentList.Count == 0)
                {
                    ...
                    if (XAbomComponentList.Count == 0)
                    {
                        ...
                        if (strheaderCreationResult.Contains("Error"))
                        {
                            ...
                            errorMessage = "Error while Creating BOM Header - Item: " + parentItem;
                            logger.Info("CreateBOMHeaderBOMComponent | CreateBOMHeader: " + errorMessage + "| Error message: " + strheaderCreationResult + Environment.NewLine);
                        ...
                        else
                        {
                            ...
                            strUserActivity = "BOM Header created for Parent Item: " + parentItem + " in XA | Env: " + environment + " | Site: " + site;
                            HelperGeneral.RecordBOMUploadToXAUserAction(loggedinUserEmail, strUserActivity, parentItem, "Site: " + site, environment);
                        }
                ...
                if (isValidItem)
                {
                    ...
                    createResult = clsXA.CreateBOMComponent(site, parentItem, parentItemRevision, alternateBOMId, componentItem, strCompParentItemRevision, usersequence, quantityPerUnit, environment, loggedinUser);
                    if (createResult.Contains("Error") || createResult.Contains("during Create"))   // CreateError
                    {
                        errorMessage = "Error while Creating BOM component along with Header: " + componentItem + "|" + createResult;
                         
                        logger.Info("CreateBOMHeaderBOMComponent | CreateBOMComponent error: " + errorMessage + Environment.NewLine);
                        boolSendEmailNotification = true;
                        logEmailBody.Append("<tr><td>" + parentItem + "</td>");
                    ...
                    if (createResult.ToUpper().Contains("SUCCESS"))
                    {
                        logger.Info("CreateBOMHeaderBOMComponent | CreateBOMComponent response: " + createResult + Environment.NewLine);
                        strUserActivity = "BOM Component created after BOM Header created for Parent Item: " + parentItem + " in XA | Env: " + environment + " | Site: " + site;
                        HelperGeneral.RecordBOMUploadToXAUserAction(loggedinUserEmail, strUserActivity, parentItem, "Site: " + site, environment);
                    }
                }
                else
                ...
                if (boolSendEmailNotification)
                {
                    ...
                    // // throw new HttpException(404, errorMessage);  NOT NEEDED 
                }
                else
                {
                    logEmailBody.Clear(); 
            ...
            catch (Exception ex)
            {
                ...
                // // errorMessage = "Error :" + ex.Message; 
                errorMessage = ""; // Suppressed error to User, since email notification sent to user and admin.
                return Json(errorMessage, JsonRequestBehavior.AllowGet);
            }
            finally
            {
                XAbomComponentList = null;
                clsXA = null;
                if (errorMessage.Contains("Error messages were generated during Create") ||
                    errorMessage.Contains("Creating BOM component") )
                {
                    errorMessage = ""; // Suppressed error to User, since email notification sent to user and admin.
                    logger.Info("CreateBOMHeaderBOMComponent|Suppressed error to User|loggedinUser: " + loggedinUser + " |LoggedinUsereMail: " + loggedinUserEmail);
                }
            }
        ...
        public ActionResult UploadBomToXA()
        {
            ...
            try
            {
                ...
                if (createHeader == "Yes")
                {
                    ...
                    if (intXABOMHeaderCnt == 0)
                    {
                        ...
                        else
                        {
                            ...
                            strUserActivity = "UpdloadBomToXA|BOM Header created for Parent Item: " + parentItemNumber + " in XA | Env: " + environment + " | Site: " + site + " | parentItemRevisionNum: " + parentItemRevisionNum;
                            HelperGeneral.RecordBOMUploadToXAUserAction(loggedinUserEmail, strUserActivity, parentItemNumber, "Site: " + site, environment); 
                        }
                ...
                {
                    // Make delay between Header creation Systemlink call and XA Header Count check 
                    System.Threading.Thread.Sleep(Convert.ToInt32(MakeDelayInProcessMilliseconds));  
                    // // _ = MakeDelayInProcessAsync(MakeDelayInProcessMilliseconds); it is not working 
                        ...
                        using (var reader = new StreamReader(Request.InputStream))
                        {
                            ...
                            foreach (Item componentItem in componentDetailList)
                            {
                                ...
                                if (componentItem.Action == "Add")
                                {
                                    ...
                                    if (intXABOMComponentCnt == 0)
                                    {
                                        ...
                                        if (createResult.ToUpper().Contains("SUCCESS")) 
                                        strUserActivity = "Add component to XA Successfully: " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber +  " | ComponentItemRevision: " + componentItem.ComponentItemRevision;
                                        else
                                            strUserActivity = "Add component to XA failed: " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | ComponentItemRevision: " + componentItem.ComponentItemRevision;
                                    }
                                }
                                else if (componentItem.Action == "Change")
                                {
                                    ...
                                    if (intXABOMComponentCnt > 0)
                                    {
                                        ...
                                        if (updateResult.ToUpper().Contains("SUCCESS")) 
                                        strUserActivity = "Change component to XA Successfully: " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | ComponentItemRevision: " + componentItem.ComponentItemRevision;
                                        else
                                            strUserActivity = "Change component to XA failed: " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | ComponentItemRevision: " + componentItem.ComponentItemRevision;
                                ...
                                else if (componentItem.Action == "Delete")
                                {
                                    ...
                                    if (intXABOMComponentCnt > 0 && !IsDontDelete)
                                    {
                                        ...
                                        if (deleteResult.ToUpper().Contains("SUCCESS")) 
                                            strUserActivity = "Delete component from XA Successfully: " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | ComponentItemRevision: " + componentItem.ComponentItemRevision;
                                        else
                                            strUserActivity = "Delete component from XA failed: " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | ComponentItemRevision: " + componentItem.ComponentItemRevision;
                                    }
                                }
                                else
                                {
                                    componentItem.Status = "Successfully Skipped/NoChange";
                                    strUserActivity = componentItem.Status + ": " + componentItem.ComponentItemNumber + " for Parent Item: " + parentItemNumber + " | ComponentItemRevision: " + componentItem.ComponentItemRevision;
                                }
                            ...
                            if (componentItem.Status != null)
                                if (!componentItem.Status.Contains("Successfully"))
                                {
                                    ...
                                    strMessageToUser = "[!] User Notification:   " + componentItem.Status;
                                    TempData["BOMUploadUserMsg"] = strMessageToUser;
                                }
                ...
                strUserActivity = "Upload BOM To XA completed for Parent Item: " + parentItemNumber;
                HelperGeneral.RecordBOMUploadToXAUserAction(loggedinUserEmail, strUserActivity, parentItemNumber, "Site: " + site, environment);
            }
            catch (Exception ex)
            {
        ...
        public void SendEmailAlertNotificationBOMUploadtoXASubject(string strEmailBody, string strToLoggedInUserEmail, string strParentItem, string strENV)
        {
            ...
            string IncludeInSkipEmail = string.Empty; 
            if (!String.IsNullOrEmpty(System.Configuration.ConfigurationManager.AppSettings["IncludeInSkipEmail"]))
                IncludeInSkipEmail = System.Configuration.ConfigurationManager.AppSettings["IncludeInSkipEmail"].ToString().Trim();

            // send email to itservice@barry-wehmiller.co if Systemlink service call error 
            if (strEmailBody.Contains("Systemlink service")) // // || strEmailBody.Contains("DB2") )  - it not needed 
            {
            ...
            // Skip header error to User JUN 2026 
            if (strEmailBody.Contains("without a structure header record") ||
                    strEmailBody.Contains("Product structure header record not found") || 
                    strEmailBody.Contains("no product structure") || 
                    strEmailBody.Contains(IncludeInSkipEmail)) 
            {
                toEmail = TOEmailITservice;
                CCEmail = CCEmailITservice;
                strEmailSub = "BOM Header missing error. Parent Item #: " + strParentItem + " | ENV: " + strENV;  
            }
